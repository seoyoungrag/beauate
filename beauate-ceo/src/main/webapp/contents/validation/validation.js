...
$.fn.emptyCheck = function() {
    ...
    if (!value) {
        alert($(this).attr('title') + '는(은) 필수입력입니다.');
        $(this).select();
        return false;
    }
    return true;
...
TypeChecker = function() {
	...
	return {
		...
		'dateText' : "날짜 형식만 가능합니다. 예)2017-01-01",
		...
		'decimalNum' : function(v) {
			if (v.trim() == "") {
				return true;
			}
			return decimalNum.test(v);
		},
		'decimalNumText' : "실수 형태만 가능합니다.(소수점 둘째짜리까지)\n 예)100, 999.99"
	};
}();
