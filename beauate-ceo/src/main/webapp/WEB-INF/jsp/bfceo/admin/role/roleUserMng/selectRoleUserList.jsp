								...
								<tbody>
									<c:forEach items="${roleInfoList}" var="list" varStatus="st">
										<tr class="row">
											...
											<td><a href="javascript:void(0);" onclick="fn_roleUserList('${list.athrCd}','${st.index}');"><c:out value="${list.athrNm}"/></a></td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
...
/* 권한별 사용자 목록 Ajax */
fn_roleUserList = function (_athrCd, _index) {
	if (_athrCd == null) {
		_athrCd = "unauthenticated";
	}
	$.ajax({
